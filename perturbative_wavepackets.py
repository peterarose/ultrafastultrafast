...
class WavepacketBuilder(HeavisideConvolve):
...
class VisualizePerturbativeWavefunctions(WavepacketBuilder):
    def __init__(self,parameter_file_path):
        self.base_path = parameter_file_path
        self.load_eigenvalues()
        self.load_eigenvectors()
        self.load_params()

        self.truncation_size = self.params['final truncation size']
        self.ground_ZPE = self.params['ground zero point energy']
        self.ground_to_excited_transition = self.params['ground to excited transition']

    def load_params(self):
        """Parameters are stored as a yaml file"""
    ...
    ### Tool for visualizing psi in the eigenbasis

    def vis_psi_in_eigenbasis(self,psi_dict):
        psi = self.psi_from_dict(psi_dict['psi'])
        manifold_num = psi_dict['manifold_num']
        psi /= np.max(np.abs(psi))
        fig, ax = plt.subplots()
        eige = self.eigenvalues[manifold_num]# + self.ground_ZPE
        for n in range(eige.size):
            ax.plot(self.t,psi[n,:] + eige)

    ### Tools for visualizing wavepackets in the spatial basis
    ### Only currently supports systems with a single vibration coordinate
    
    def SHO_basis(self,n,xvalues):
        coef = [0 for i in range(n)]
        coef.append(1)
        norm = 1/(np.pi**(1/4)*np.sqrt(2**n * factorial(n)))
        return  norm * np.exp(-xvalues**2/2) * hermval(xvalues,coef)

    def oscillator_1d(self,array_1d,x):
        ans = np.zeros(x.shape,dtype=array_1d.dtype)
        for i in range(len(array_1d)):
            ans += self.SHO_basis(i,x) * array_1d[i]
        return ans

    def oscillator_1d_b(self,array_1d,x):
        norm_vec = np.array([1/(np.pi**(1/4)*np.sqrt(2**n * factorial(n))) for n in range(array_1d.size)])
        coef = array_1d * norm_vec
        ans = hermval(x,coef) * np.exp(-x**2/2)
        return ans

    def calculate_psi_of_x(self,psi,*,x=np.arange(-1,3.01,.1),manifold_num = 0):
        eigv = self.eigenvectors[manifold_num]

        psi = eigv.dot(self.psi_from_dict(psi))
        num_sites = len(self.energies[manifold_num])
        reshaping_vib = [self.truncation_size for n in range(self.num_vibrations)]
        reshaping_vib.insert(0,num_sites)
        reshaping_vib.append(self.t.size)
        psi = psi.reshape(tuple(reshaping_vib))

        # vibrational_energies = []
        # for s in range(num_sites):
        #     psi_site = psi[s,:self.eigenvalues[0].size,-1]

        #     vib_en = np.real(np.dot(np.conjugate(psi_site),psi_site*self.eigenvalues[0]))
        #     vib_en /= np.real(np.dot(np.conjugate(psi_site),psi_site))
        #     vibrational_energies.append(vib_en)

        self.x = x

        psi_of_x = np.zeros((num_sites,x.size,self.t.size),dtype='complex')
        for n in range(self.t.size):
            for s in range(num_sites):
                psi_of_x[s,:,n] = self.oscillator_1d_b(psi[s,:,n],x)

        return psi_of_x#, vibrational_energies

    def vis_psi_of_x(self,psi_of_x,vib_energies,*,manifold_num=0):
        print(vib_energies)

        num_sites = psi_of_x.shape[0]
        fig, axes = plt.subplots(num_sites)
        if num_sites == 1:
            axes = [axes]
        else:
            pass
        ...
        if manifold_num == 0:
            ...
            d_list = self.params['vibrations'][0]['displacements1']
        else:
            d_list = self.params['vibrations'][0]['displacements2']
        for s in range(num_sites):
            pot = (self.x-d_list[s])**2
            axes[s].plot(self.x,pot)
            en = vib_energies[s]# + self.ground_ZPE
            la, = axes[s].plot(self.x,np.abs(psi_of_x[s,:,0])+en)
            la_list.append(la)
            # lr, = axes[s].plot(self.x,np.real(psi_of_x[s,:,0])+en)
            # lr_list.append(lr)
            # li, = axes[s].plot(self.x,np.imag(psi_of_x[s,:,0])+en)
            # li_list.append(li)
            

            max_val = np.max(np.abs(psi_of_x[s,...])+en)
            axes[s].set_ylim([-1,max_val])

        @interact(t=widgets.FloatSlider(min=-10,max=30,step=(self.t[1]-self.t[0]),
                                        value=0,layout=widgets.Layout(width='800px') ) )
        def update(t):
            t_ind, t = self.get_closest_index_and_value(t,self.t)
            for s in range(num_sites):
                en = vib_energies[s]# + self.ground_ZPE
                # lr_list[s].set_ydata(np.real(psi_of_x[s,:,t_ind])+en)
                # li_list[s].set_ydata(np.imag(psi_of_x[s,:,t_ind])+en)
                la_list[s].set_ydata(np.abs(psi_of_x[s,:,t_ind])+en)
